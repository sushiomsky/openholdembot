...
bool CAutoplayer::ExecuteSecondaryFormulasIfNecessary() {
  ...
  // We don't deal with f$terminate here,
  // as we only terminate if we are not connected,
  // but then the autoplayer is disabled.
  // The heartbeat-loop is a better place for that  
